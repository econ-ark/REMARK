...
class PortfolioSolution(Solution):
    ...
    cFuncAdjHabit : Interp2D
        Consumption function with habits over normalized market resources when the agent is able
        to adjust their portfolio shares.
    # ShareFuncAdj : Interp1D
        Risky share function over normalized market resources when the agent is able
        to adjust their portfolio shares.
    ShareFuncAdjHabit : Interp2D
        Risky share function with habits over normalized market resources when the agent is able
        to adjust their portfolio shares.
    # vFuncAdj : ValueFunc
        Value function over normalized market resources when the agent is able to
        adjust their portfolio shares.
    vFuncAdjHabit : ValueFunc
        Value function with habits over normalized market resources when the agent is able to
        adjust their portfolio shares.
    # vPfuncAdj : MargValueFunc
        Marginal value function over normalized market resources when the agent is able
        to adjust their portfolio shares.
    vPfuncAdjHabit : MargValueFunc
        Marginal value function with habits over normalized market resources when the agent is able
        to adjust their portfolio shares.    
    cFuncFxd : Interp2D
    ...
    def __init__(self,
        ...
        if cFuncAdj is None:
            cFuncAdj = NullFunc()
        if cFuncAdjHabit is None:
            cFuncAdjHabit = NullFunc()
        if cFuncFxd is None:
            cFuncFxd = NullFunc()
        if ShareFuncAdj is None:
            ShareFuncAdj = NullFunc()
        if ShareFuncAdjHabit is None:
            ShareFuncAdjHabit = NullFunc()
        if ShareFuncFxd is None:
            ShareFuncFxd = NullFunc()
        if vFuncAdj is None:
            vFuncAdj = NullFunc()
        if vFuncAdjHabit is None:
            vFuncAdjHabit = NullFunc()
        if vFuncFxd is None:
            vFuncFxd = NullFunc()
        if vPfuncAdj is None:
            vPfuncAdj = NullFunc()
        if vPfuncAdjHabit is None:
            vPfuncAdjHabit = NullFunc()
...
class PortfolioConsumerType(IndShockConsumerType):
    ...
    def updateSolutionTerminal(self):
        '''
        Solves the terminal period of the portfolio choice problem with internal habit.  The solution is
        trivial, as usual: consume all market resources, and put nothing in the risky asset (because you have nothing anyway).
        From GM:"In the last period the policy functions are trivial (the agent consumes all available wealth) and the value 
        function corresponds to the indirect utility function. "
        Parameters
        ----------
        None
        ...
        # let c=market resource, and the market resource is the first argument, soe i_dim=0
        cFuncAdjHabit_terminal = IdentityFunction(i_dim=0, n_dims=2)
        # you need Fxd since if you have not enter the stock market your choice of risky share is fixed, or this is used
        # in the example for sticky type, see ipynb.
        
        # Risky share is irrelevant-- no end-of-period assets; set to zero
        ShareFuncAdj_terminal = ConstantFunction(0.)
        # Here you cannot change the risky share in terminal,so it is sill the fixed share, and that
        # is the second argument so i_dim=1
        ShareFuncFxd_terminal = IdentityFunction(i_dim=1, n_dims=2)
        ShareFuncAdjHabit_terminal = ConstantFunction(0.)
        
        # Habit
        HabitFunc_terminal = ConstantFunction(1.)
        ...
        # 當時2D的value function就定義了只需要一個function,就是CRRA裏面放什麽
        # Value function is the utility from consuming market resources comparing to multiplicative habits
        vFuncAdjHabit_terminal = ValueFunc2D((cFuncAdjHabit_terminal/(HabitFunc_terminal), self.CRRA)
        
        # Marginal value of market resources is marginal utility at the consumption function
        ...
        vPfuncAdj_terminal = MargValueFunc((cFuncAdjHabit_terminal/HabitFunc_terminal), self.CRRA)
        
        # Construct the terminal period solution
        self.solution_terminal = PortfolioSolution(
                cFuncAdj=cFuncAdjHabit_terminal,
                ShareFuncAdj=ShareFuncAdjHabit_terminal,
                vFuncAdj=vFuncAdjHabit_terminal,
                vPfuncAdj=vPfuncAdjHabit_terminal,
...
def solveConsPortfolio(solution_next,ShockDstn,IncomeDstn,RiskyDstn,
    ...
    # same things in ConIndShockModel line 399 defUtilityFuncs(self)
    u = lambda x : utility(x/h, CRRA)
    uP = lambda x : utilityP(x/h, CRRA)
    uPinv = lambda x : utilityP_inv(x/h, CRRA)
    n = lambda x : utility_inv(x/h, CRRA)
    nP = lambda x : utility_invP(x/h, CRRA)
    ...
    if IndepDstnBool: # If the distributions ARE independent...
        ...
        # Calculat future realizations of habits
        hNrm_next = cNrmAdjHabit_now                                         
        
        #先不管                                     
        # Evaluate realizations of marginal value of market resources next period
        # dvdmAdj_next = vPfuncAdj_next(mNrm_next)
        dvdmAdjHabit_next = vPfuncAdjHabit_next(mNrm_next)
        if AdjustPrb < 1.:
        ...
        dvdsAdjHabit_next = np.zeros_like(mNrm_next) # No marginal value of Share if it's a free choice!
        if AdjustPrb < 1.:
        ...
        if vFuncBool:
            ...
            vAdjHabit_next = vFuncAdjHabit_next(mNrm_next)
            if AdjustPrb < 1.:
    ...
    if vFuncBool:
        ...
        vFuncAdj_now = None
        vFuncAdjHabit_now = None
        vFuncFxd_now = None

    # Create and return this period's solution
    return PortfolioSolution(
            cFuncAdj = cFuncAdj_now,
            cFuncAdjHabit = cFuncAdjHabit_now,
            ShareFuncAdj = ShareFuncAdj_now,
            ShareFuncAdjHabit = ShareFuncAdjHabit_now,
            vPfuncAdj = vPfuncAdj_now,
            vPfuncAdjHabit = vPfuncAdjHabit_now,
            vFuncAdj = vFuncAdj_now,
            vFuncAdjHabit = vFuncAdjHabit_now,
