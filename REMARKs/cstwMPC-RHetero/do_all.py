'''
Replication of the paper "Can Persistent Unobserved Heterogeneity in Returns-to-Wealth Explain Wealth Inequality?" 
'''
...
import Calibration.SetupParams as Params
...
warning_message = """\n\nWARNING: you have selected to run the model with ex-ante heterogeneity, with RAM < """+ str(desired_RAM_size_GB) +"""GB. This may fill up your memory and crash your computer. Please run this code on a machine with >= """+ str(desired_RAM_size_GB) +"""GB of RAM. 
You may want to interrupt this code to avoid experiencing an out-of-memory computer crash.\n\n"""

if actual_RAM_size_GB < desired_RAM_size_GB:
    warnings.warn(warning_message)
    
my_file_path = os.path.dirname(os.path.abspath(__file__))
figures_dir = os.path.join(my_file_path, "Results/") # Relative directory for primitive parameter files

#Define Agent and Market Classes
EstimationAgentClass = Model.IndShockConsumerType
EstimationMarketClass = Market

class cstwMPCagent(Model.KinkedRconsumerType):
    '''
...
class cstwMPCmarket(EstimationMarketClass):
    ...
    def showManyStats(self,spec_name=None):
        ...
        if spec_name is not None:
            with open(self.my_file_path  + '/Results/' + 'RandBetaHeteroResults.txt','w') as f:
...
# Returns to Wealth Heterogeneity with Preference Heterogeneity
def main():
