'''
This module executes the lifecycle model in cstwMPC paper without heterogenous impatience.
...
print("The execution takes about 40 minutes with a RAM of 16GB and a processor of 2.5 GHz on macOS High Sierra")
...
class cstwMPCmarket(EstimationMarketClass):
    ...
    def showManyStats(self,spec_name=None):
        ...
        plt.plot(LorenzAxis,self.LorenzData,'-k', label='LifeCycle', linewidth=1.5)
        ...
        plt.legend(loc=2)
        plt.ylim([-0.02,1.0])
        plt.savefig(my_file_path  + '/Figures/LifeCycleBetaPoint.pdf')
        plt.show()
        ...
        if spec_name is not None:
            with open(self.my_file_path  + '/Results/' + 'LifeCycleBetaPoint.txt','w') as f:
