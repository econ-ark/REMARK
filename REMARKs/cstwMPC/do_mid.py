'''
This module estimates the mean and spread of the distribution of the impatience across households.
Then it compares the model implied wealth distribution with the data.
...
print("The execution takes about 30 minutes with a RAM of 16GB and a processor of 2.5 GHz on macOS High Sierra")
...
class cstwMPCmarket(EstimationMarketClass):
    ...
    def showManyStats(self,spec_name=None):
        ...
        plt.plot(LorenzAxis,self.LorenzData,'-k', label=r'$\beta$ -Dist', linewidth=1.5)
        plt.plot(LorenzAxis,LorenzSim,'--k', label='US data (SCF)', linewidth=1.5)
        plt.xlabel('Income percentile',fontsize=12)
        plt.ylabel('Cumulative wealth share',fontsize=12)
        plt.legend(loc=2)
        plt.ylim([-0.02,1.0])
        plt.savefig(my_file_path  + '/Figures/BetaDist.pdf')
        plt.show()
        ...
        if spec_name is not None:
            with open(self.my_file_path  + '/Results/' + 'BetaDist' + 'Results.txt','w') as f:
