'''
This module executes the model in cstwMPC paper with shocks at the aggregate level with heterogenous impatience.
...
class cstwMPCmarket(EstimationMarketClass):
    ...
    def showManyStats(self,spec_name=None):
        ...
        plt.legend(loc=2)
        plt.ylim([-0.02,1.0])
        plt.savefig(my_file_path  + '/Figures/AggregateShocksBetaDist.pdf')
        plt.show()
        ...
        if spec_name is not None:
            with open(self.my_file_path  + '/Results/' + 'AggregateShocksBetaDist.txt','w') as f:
