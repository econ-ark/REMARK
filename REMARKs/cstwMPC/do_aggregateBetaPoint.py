'''
This module executes the model in cstwMPC paper with shocks at the aggregate level without heterogenous impatience.
...
print("The execution takes about 2 hours with a RAM of 16GB and a processor of 2.5 GHz on macOS High Sierra")
...
class cstwMPCmarket(EstimationMarketClass):
    ...
    def showManyStats(self,spec_name=None):
        ...
        plt.plot(LorenzAxis,self.LorenzData,'-k', label=r'$\beta$ -Point', linewidth=1.5)
        ...
        plt.legend(loc=2)
        plt.ylim([-0.02,1.0])
        plt.savefig(my_file_path  + '/Figures/AggregateShocksBetaPoint.pdf')
        plt.show()
        ...
        if spec_name is not None:
            with open(self.my_file_path  + '/Results/' + 'AggregateShocksBetaPoint.txt','w') as f:
