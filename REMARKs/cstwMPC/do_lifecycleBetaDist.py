'''
This module executes the lifecycle model in cstwMPC paper with heterogenous impatience.
...
print("The execution takes about ... hours with a RAM of 16GB and a processor of 2.5 GHz on macOS High Sierra")
...
class cstwMPCmarket(EstimationMarketClass):
    ...
    def showManyStats(self,spec_name=None):
        ...
        plt.legend(loc=2)
        plt.ylim([-0.02,1.0])
        plt.savefig(my_file_path  + '/Figures/LifeCycleBetaDist.pdf')
        plt.show()
        ...
        if spec_name is not None:
            with open(self.my_file_path  + '/Results/' + 'LifeCycleBetaDist.txt','w') as f:
