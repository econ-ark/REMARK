'''
This module shows the wealth distribution comparison between the models with and without heterogenous impatience. 
Descriptive statistics are saved in the Results directory and wealth distribution graphs are both output to screen
and saved in the Figures directory.
The problem is solved without aggregate shocks.
'''
...
import Code.cstwMPCparams as Params
...
print("The execution takes about 1 minute with a RAM of 16GB and a processor of 2.5 GHz on macOS High Sierra")
...
# This part constructs seven instances of IndShockConsumerType with different discount factors
BaselineType = IndShockConsumerType(**Params.init_do_1)
# Specify the distribution of the discount factor
num_types = 7              # number of types we want
DiscFac_mean   = 0.986609223266 # center of beta distribution 
DiscFac_spread = 0.00853886395698 # spread of beta distribution
...
lorenz_sim = getLorenzShares(aLvl_sim,percentiles=Params.percentiles_to_match)
pctiles = np.linspace(0,1.0,101)
...
#    lorenz_SCF = np.array([-0.00183091,  0.0104425 ,  0.0552605 ,  0.1751907 ])
    lorenz_SCF = getLorenzShares(Params.SCF_wealth,weights=Params.SCF_weights,percentiles=Params.percentiles_to_match)
    aLvl_sim = np.concatenate([ThisType.aLvlNow for ThisType in MyTypes])
    lorenz_sim = getLorenzShares(aLvl_sim,percentiles=Params.percentiles_to_match)
    lorenz_distance = np.sqrt(np.sum((100*(lorenz_SCF - lorenz_sim))**2))
...
#        self.print_parallel_error_once = False
        ...
        self.AggShockBool = do_agg_shocks
    ...
    def showManyStats(self,spec_name=None):
        ...
        plt.plot(LorenzAxis,LorenzSim,'--k', label=r'$\beta$ -Point', linewidth=1.5)
        plt.plot(LorenzAxis,sim_Lorenz_points,'--r', label=r'$\beta$ -Dist', linewidth=1.5)
        plt.plot(LorenzAxis,self.LorenzData,'-k', label='US data (SCF)', linewidth=1.5)
        plt.xlabel('Income percentile',fontsize=12)
        plt.ylabel('Cumulative wealth share',fontsize=12)
        plt.legend(loc=2)
        plt.ylim([-0.02,1.0])
        plt.savefig(my_file_path  + '/Figures/BetaPointvsBetaDist.pdf')
        plt.show()
...
#        print(results_string)
        ...
        if spec_name is not None:
            with open(self.my_file_path  + '/Results/' + 'BetaPoint' + 'Results.txt','w') as f:
...
def main():
    ...
    EstimationEconomy.Population = Population
    EstimationEconomy.AggShockBool = do_agg_shocks
...
#            t_end = clock()
        ...
        EstimationEconomy.distributeParams(Params.param_name,pref_type_count,center_estimate,spread_estimate,Params.dist_type)
...
#        print('Estimate is center=' + str(center_estimate) + ', spread=' + str(spread_estimate) + ', took ' + str(t_end-t_start) + ' seconds.')
        ...
        t_End = perf_counter()
        time_to_estimate = t_End - t_Begin
        print('Results for Beta-Dist')
        print('Estimate is center = ' + str(DiscFac_mean) + ', spread = ' + str(DiscFac_spread))
        print('Lorenz distance is ' + str(calcLorenzDistance(MyTypes)))
