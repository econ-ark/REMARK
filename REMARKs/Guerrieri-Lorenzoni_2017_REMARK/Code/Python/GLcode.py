...
#Note** Underneath each class are the functions that were edited. 
#If a function in the original ConMarkovModel.py was edited,
#then it will likely have a GL at the end of its name. 
...
class GLSolver(ConsIndShockSolver):
    ...
    def getPointsForInterpolationGL(self, EndOfPrdvP, aNrmNow):
        """
        Finds interpolation points (c,b) for the consumption function.
        Parameters
        ----------
        EndOfPrdvP : np.array
        ...
        #Tile the asset grid constructed under calcEndOfPrdvPGL 
        self.Bgrid_rep=np.tile(self.Bgrid,(13,1))
              
        #Endogenous Gridpoints Method
        cNow = self.uPinv(EndOfPrdvP) #Consumption FOC
    ...
    def calcEndOfPrdvPGL(self):
        ...
        for k in range(aNrmMin_unique.size):
           ...
           # Construct Asset Grid 
            ...
            Bgrid_uc=Bgrid_uc[(Bgrid_uc>self.BoroCnstArt)]
...
class GLConsumerType(IndShockConsumerType):
    ...
    def updateSolutionTerminal(self):
        ...
        none
        """
        print("It should take 8 Seconds to solve this Agent's problem ")
