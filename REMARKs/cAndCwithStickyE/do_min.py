...
import subprocess
from Code.StickyEmodel import StickyEmarkovConsumerType, StickyEmarkovRepAgent, StickyCobbDouglasMarkovEconomy
from HARK.ConsumptionSaving.ConsAggShockModel import SmallOpenMarkovEconomy
from HARK.utilities import plotFuncs
import matplotlib.pyplot as plt
import Code.StickyEparams as Params
from Code.StickyEtools import makeStickyEdataFile, runStickyEregressions, makeResultsTable,\
...
if __name__ == '__main__':
    ...
    if make_emp_table:
        ...
        cmd = [Params.stata_exe, "do", empirical_dir + "_usConsDynEmp.do", base_path]
        # Run Stata do-file
    ...
    if make_histogram:
        # Define the command to run the Stata do file
        base_path = os.path.abspath('../../')
        cmd = [Params.stata_exe, "do", empirical_dir + "metaAnalysis/habitsHistogram.do", base_path]
        # Run Stata do-file
        stata_status = subprocess.call(cmd,shell = 'true')
        if stata_status!=0:
            raise ValueError('Stata code could not run. Check the stata_exe in StickyEparams.py')
