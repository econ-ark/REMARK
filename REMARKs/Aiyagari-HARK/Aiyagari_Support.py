...
init_Aiyagari_agents = {
    ...
    "LaborSD": 0.3,             #0.3
    "T_cycle": 1,
    "DiscFac": 0.96,            #KS:0.99    #Aiyagari: 0.96                                                         #!KS
    "CRRA": 1.0,
...
class AiyagariType(AgentType):
    ...
    def precompute_arrays(self):
        ...
        KnextB = np.tile(np.reshape(AnowB, [1, Mcount, 1, 1]), [1, 1, 1, 4*self.LaborStatesNo])
        KnextG = np.tile(np.reshape(AnowG, [1, Mcount, 1, 1]), [1, 1, 1, 4*self.LaborStatesNo])
        Knext = np.concatenate((KnextB, KnextB, KnextG, KnextG, KnextB, KnextB, KnextG, KnextG, KnextB, KnextB, KnextG, KnextG, KnextB, KnextB, KnextG, KnextG, KnextB, KnextB, KnextG, KnextG, KnextB, KnextB, KnextG, KnextG, KnextB, KnextB, KnextG, KnextG), axis=2)    #! This needs adaptation if LaborStatesNo is changed
        ...
        #!N Per Aiyagari state there are four states: Bad-unempl, Bad-empl, Good-unempl, Good-empl
...
#! TODO: Create if check to ensure AgentCount/LaborStatesNo is an integer
    ...
    def get_shocks(self):
        ...
        # Indicator for Aggregate state of economy; derive from past employment rate rather than store previous value
        ...
        LSStateNow = [int(element) for element in self.state_prev["LaborSupplyState"]]


        #Shocking agents' idiosyncratic labor supply given the tauchen approximation of the AR(1) process:
        for i in range(len(LSStateNow)):
            LSStateNow[i] = np.random.choice(range(self.LaborStatesNo), size=None, replace=True, p=transition_prob[LSStateNow[i]])
...
def solve_Aiyagari(
    ...
    cNow = np.concatenate([np.zeros([1, Mcount, 4*n]), cNow], axis=0)
    mNow = np.concatenate([np.zeros([1, Mcount, 4*n]), mNow], axis=0)
...
init_Aiyagari_economy = {
    ...
    "UrateB": 0.1,      #Original KS value 0.10     #Aiyagari: 0                                                    #!KS
    "UrateG": 0.04,      #Original KS value 0.04     #Aiyagari: 0                                                    #!KS
    "RelProbBG": 0.75,  #0.75
    "RelProbGB": 1.25,  #1.25
    "MrkvNow_init": 0,
}
...
#######################################################################################
        ...
        #Markov matrix given by the tauchen approximation by the respective probabilities of transitioning from one aggregate-
