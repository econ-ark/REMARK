...
# <!-- [![Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/econ-ark/REMARK/master?filepath=REMARKs%2FPortfolioChoiceBlogPost%2FPortfolioChoiceBlogPost.ipynb) -->
#
# Economists like to compare actual human behavior to choices that would be made by a "rational" agent who takes into account all the complexities of a decision.
#
# But for some problems, calculating the optimal choice is remarkably difficult.  
#
# Lifetime financial decisions are such a problem.  Determining the optimal amount to save for retirement, and how much of your savings to invest in risky assets (like stocks) versus safe assets (say, government bonds), turns out to be mathematically **much** harder than calculating how to land the Apollo spacecraft on the moon.  In fact, the computational tools that economists use to solve such problems descend directly from those originally developed to optimize Apollo trajectories -- with 50 years of further development.
#
...
#
# But even today, these tools are not widely used, because it takes years of study to master them.  
#
# In 2015, the U.S. [Consumer Financial Protection Bureau](https://www.consumerfinance.gov) funded the creation of the [Econ-ARK](https://econ-ark.org) open source software project, whose purpose is to make such tools much more accessible, to scholars and to the wider public.  Thanks to subsequent [funding by the Sloan Foundation](https://sloan.org) and the [Think Forward Initiative](https://www.thinkforwardinitiative.com), the Econ-ARK team is proud to announce [the `ConsPortfolioModel` tool](https://hark.readthedocs.io/en/latest/example_notebooks/ConsPortfolioModel.html) which can calculate the optimal solution to a lifetime optimal saving problem where the consumer can choose how much to invest in risky versus safe assets.
#
...
#
# Uncertainties like this are why a household's retirement savings problem is so much harder than NASA's moonshot. The motion of a spacecraft is predictable:  Point it in a certain direction with a certain velocity, and Newton's equations say where it will be far into the future.  In contrast, "optimal" behavior over a lifetime that is vulnerable to many risks must prudently take into account all of the possible outcomes.  
#
...
#
# The most striking conclusion of the CGM paper is captured in figure 1.  Our toolkit assumes that consumers with risk aversion of 3 can choose between a "risky" asset with expected performance (for risk and return) like the stock market, versus a "safe" asset with lower  returns historically typical of safe assets.  The figure shows, by age, the optimal risky share -- that is, the proportion of savings that it would be optimal to invest in the "risky" asset.  The fact that the proportion is stuck at 1.0 at every age means that the computer says the optimal choice is always to invest 100 percent of your savings in stocks!
#
# <center>
...
#
# The calculations behind the figure assume that people expect an equity premium of 4 percent, which is [a good estimate](http://breesefine7110.tulane.edu/wp-content/uploads/sites/110/2015/10/Perspectives-on-the-Equity-Risk-Premium-Siegel.pdf) of what the average premium has been on stock market investments in the developed world over the past century.  (Risk is also assumed to match the historical average.)
#
# The computer's conclusion is that for values of risk aversion that accurately capture people's risk-related choices in other contexts, an equity premium of 4 percent is more than enough to compensate any rational agent for the risk that has typically been associated with stock returns.

# %% [markdown]
# ## Maybe Risk Aversion is Much Greater than 3?
#
# Parameters like "relative risk aversion" are hard to measure.  Maybe the conventional choice of 3, which works well to explain other choices, is inappropriate here -- maybe people just hate stock market risk more than other kinds of risk that would have similar financial consequences.
#
...
# 1. By the age of retirement, you plan to finance _a lot_ of your future spending from your savings
...
#
# Figure 3 below shows the consequences if highly risk averse people believe the equity premium will be only two percent (which is around its historical average in the U.S. before 1941, and which [some respected economists](https://jpm.pm-research.com/content/26/1/10.abstract) think might prevail in the future).
#
...
# #### Is Pessimism Enough?
#
# The preceding figure assumes that relative risk aversion is very high (6).  A natural question is whether, when people are pessimistic about the equity premium, their optimal portfolio shares might be low even if they are not so risk averse.  
#
...
# %% [markdown]
# Investment advisors sometimes advocate the "100 minus age" rule: The percentage of your portfolio in risky assets should be 100 minus your age; so, a 60 year old would have 100-60=40 percent in stocks.
#
# For people before retirement, the rule's recommendation is not too different from what comes out of the model (assuming high risk aversion of 6).  While the rule would say that the 25 year old should put 75 percent of their savings in the stock market and the model says 100 percent, they agree that the young person's proportion should be high, and also agree that the proportion should decline during working life.
#
...
#
# For technical reasons, it is difficult for the model to incorporate risk aversion that increases with age.  But your willingness to invest in risky assets depends on both your degree of aversion to risk and your perception of the size of the risk.  So a backdoor way to examine the consequences of rising risk aversion with age is to assume that the _perceived_ riskiness of stock investments goes up with age.  
#
# That is what is done in figure 5 below: We assume that the perceived riskiness of stock market investment doubles between age 65 and age 100.  The result now looks more like the advice of financial advisors:  Increasing _perceived_ risk as you get older persuades you to invest less in risky assets.
#
...
# ### Other Experiments
#
# Many other experiments are possible in the framework (as you can discover yourself by downloading the tool; for instructions, see below), but the conclusion is always the same: Even if people expect that stock returns in the future will be substantially lower than they have been in the past, for most people over most of their lifetime, the return on stock market investments more than compensates for any reasonable degree of risk aversion.
