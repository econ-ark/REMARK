...
# \end{eqnarray}
#
# When the consumer has a CRRA utility function $u(c)=\frac{c^{1-ρ}}{1-ρ}$, the paper shows that the problem can be written in terms of ratios of money variables to permanent income, e.g. $m_{t} \equiv M_{t}/P_{t}$, and the Bellman form of [the problem reduces to](http://econ.jhu.edu/people/ccarroll/papers/BufferStockTheory/#The-Related-Problem):
#
# \begin{eqnarray*}
...
#
# Show numerically the result that is proven analytically in [The-Liquidity-Constrained-Solution-as-a-Limit](http://econ.jhu.edu/people/ccarroll/papers/BufferStockTheory/#The-Liquidity-Constrained-Solution-as-a-Limit), by solving the model for successively smaller values of $℘$.
...
#
# An analogous condition can be defined when there is uncertainty about permanent income.  Defining $\tilde{Γ} = (\mathbb{E}[ψ^{-1}])^{-1}Γ$, the 'Growth Impatience Condition' (GIC) is that 
# \begin{eqnarray}
#   \Phi/\tilde{Γ} & < & 1
# \end{eqnarray}
...
# \end{eqnarray}
#
# That is, as $℘$ approaches zero the problem with uncertainty becomes identical to the problem that instead has constraints.  (See [Precautionary Saving and Liquidity Constraints](http://econ.jhu.edu/people/ccarroll/papers/LiqConstr) for a full treatment of the relationship between precautionary saving and liquidity constraints).
...
# ### SOLUTION - Analysis
#
# Defining $\tilde{Γ} = (\mathbb{E}[ψ^{-1}])^{-1}Γ$, substituting for $\Phi$ in the formula above, the 'Growth Impatience Condition' (GIC) is that 
#
# \begin{eqnarray}
#   (𝖱 β)^{1/ρ} & < & \tilde{Γ} \\
#   (𝖱 β) & < & \tilde{Γ}^{ρ} \\
